...
const IGNORED_BLOCK_IDS = new Set([209, 518, 513, 514, 182, 208, 271, 610, 601, 427, 211, 605, 246, 520, 585, 514]);
...
export const getDistance3D = (p1, p2) => Math.hypot(p1.x - p2.x, p1.y - p2.y, p1.z - p2.z);
export const getDistance2D = (p1, p2) => Math.hypot(p1.x - p2.x, p1.z - p2.z);

export const adjustSplineToEyeLevel = (spline) => spline.map((node) => ({ ...node, y: node.y + EYE_HEIGHT - 2 }));
...
function getBlockHeight(block) {
    ...
    if (registryName.includes('slab') && !registryName.includes('double')) return 0.5;
    if (registryName.includes('snow_layer') || (registryName.includes('snow') && !registryName.includes('block'))) return 0.5;
    if (registryName.includes('farmland') || registryName.includes('grass_path') || registryName.includes('dirt_path')) return 0.9375;
    if (registryName.includes('carpet')) return 0.0625;
    if (registryName.includes('pressure_plate') || registryName.includes('button')) return 0.0625;
...
export function canSeePoint(point, eyePos) {
    ...
    const blocksInPath = RayTrace.rayTraceBetweenPoints([eyePos.x, eyePos.y, eyePos.z], [point.x, point.y, point.z]);

    return !blocksInPath.some((blockData) => {
        const [x, y, z] = blockData.map(Math.floor);

        if (Math.abs(x - pX) <= 1 && Math.abs(z - pZ) <= 1 && (y === pY || y === pY - 1 || y === pY - 2)) return false;
        ...
        if (blockId !== 0 && !IGNORED_BLOCK_IDS.has(blockId)) {
            ...
            if (DEBUG_MODE) Chat.message(`§cBlocked by block at ${x}, ${y}, ${z} - NAME: ${block.type.getRegistryName()} - ID: ${blockId}`);
            return true;
        }
        return false;
    });
...
export function canPhysicallyReach(point, playerPos, isJumping) {
    if (isJumping) {
        ...
        if (horizontalDist > 3) {
            ...
            const angleDiff = Math.abs(((angleToPoint - currentYaw + 540) % 360) - 180);

            if (angleDiff > 30) {
                if (DEBUG_MODE) Chat.message(`§7[Reach] Point at ${angleDiff.toFixed(0)}° during jump - too sharp`);
                return false;
            }
        }
    }
...
export function updateCurrentNode() {
    ...
    if (movementState.recoveryLockTicks > 0) {
        ...
        if (DEBUG_MODE && movementState.recoveryLockTicks === 0) {
            Chat.message(`§a[Recovery] Lock released, resuming normal navigation`);
        }
        return;
    }

    const playerPos = { x: Player.getX(), y: Player.getY(), z: Player.getZ() };
    const isJumping = movementState.jumpTriggered || movementState.fallingTicks > 0;

    const useVertical = movementState.fallingTicks <= 2 && !movementState.anticipatingFall;
    ...
    while (movementState.currentNodeIndex < movementState.splinePath.length - 1) {
        ...
        const nextNode = movementState.splinePath[movementState.currentNodeIndex + 1];
        ...
        if (distanceFunc(eyePos, currentNode) < NODE_REACH_DISTANCE) {
            ...
            if (distToNext < NODE_REACH_DISTANCE || distToNext > VISIBILITY_LOOKAHEAD) {
    ...
    const isStable = movementState.fallingTicks === 0 && !movementState.jumpTriggered;

    if (isOnGround && isStable && movementState.ticksWithoutProgress < 10) {
        const MAX_SEARCH_AHEAD = 10;
        const endIndex = Math.min(movementState.currentNodeIndex + MAX_SEARCH_AHEAD, movementState.splinePath.length);
        ...
        for (let i = movementState.currentNodeIndex + 1; i < endIndex; i++) {
            ...
            if (distToPoint < NODE_REACH_DISTANCE && canPhysicallyReach(point, playerPos, false)) {
                if (DEBUG_MODE) Chat.message(`§bSkipping ahead to node ${i} (was at ${movementState.currentNodeIndex})`);
...
export function calculateLookaheadTarget() {
    ...
    if (currentNodeIndex >= splinePath.length - 1) return splinePath[splinePath.length - 1];

    const playerPos = { x: Player.getX(), y: Player.getY(), z: Player.getZ() };
    const isJumping = movementState.jumpTriggered || movementState.fallingTicks > 0;

    const speed = getPlayerSpeed();
    let dynamicLookahead = Math.min(LOOK_AHEAD_BASE + speed * 2, LOOK_AHEAD_MAX);
    ...
    for (let i = currentNodeIndex; i < splinePath.length - 1; i++) {
        ...
        if (!canPhysicallyReach(p2, playerPos, isJumping)) {
            if (DEBUG_MODE && i > currentNodeIndex) {
                Chat.message(`§7[Lookahead] Stopped at node ${i}, can't reach ${i + 1}`);
            }
            break;
        }
...
export function isAtFinalNode() {
    const playerPos = { x: Player.getX(), y: Player.getY(), z: Player.getZ() };
    const lastNode = movementState.splinePath[movementState.splinePath.length - 1];
    return getDistance3D(playerPos, lastNode) < FINAL_NODE_THRESHOLD;
}

export function findClosestNodeIndex(splinePath) {
    const playerPos = { x: Player.getX(), y: Player.getY(), z: Player.getZ() };
    return splinePath.reduce((closest, node, i) => (getDistance3D(playerPos, node) < closest.dist ? { i, dist: getDistance3D(playerPos, node) } : closest), {
        i: 0,
        dist: Infinity,
    }).i;
}
...
function canAttemptJump(player) {
    ...
    const { splinePath, currentNodeIndex, lastJumpPos, ticksSinceLanding } = movementState;
    if (!splinePath || !splinePath.length || currentNodeIndex >= splinePath.length - 1) return false;

    if (lastJumpPos) {
        const playerPos = { x: Player.getX(), z: Player.getZ() };
        if (Math.hypot(playerPos.x - lastJumpPos.x, playerPos.z - lastJumpPos.z) < 0.5) return false;
    }

    return !movementState.jumpTriggered && (ticksSinceLanding === null || ticksSinceLanding >= 2);
}
...
function getDirection(from, to) {
    ...
    return dist2D < 0.1 ? { dirX: 0, dirZ: 0 } : { dirX: dx / dist2D, dirZ: dz / dist2D };
}

function evaluateStepJump(checkX, checkY, checkZ, currentGroundHeight, playerPos) {
    ...
    if (actualHeightDiff > STEP_HEIGHT) {
        if (DEBUG_MODE) {
            ...
            Chat.message(`§e[Jump] Step +${actualHeightDiff.toFixed(2)} (${footName})`);
        }
    ...
    if (DEBUG_MODE) Chat.message(`§7[No Jump] Can step over ${actualHeightDiff.toFixed(2)} blocks`);
    return false;
}

function evaluateGapJump(checkX, checkY, checkZ, pX, pY, pZ, currentGroundHeight, playerPos, splinePath, currentNodeIndex) {
    const blockFoot = getCachedBlock(checkX, checkY, checkZ);
    ...
    if (actualHeightDiff > -STEP_HEIGHT) {
        const nextNode = splinePath[Math.min(currentNodeIndex + 1, splinePath.length - 1)];
        const immediateYDiff = nextNode.y - playerPos.y;

        if (immediateYDiff > STEP_HEIGHT || fallDepth > 2) {
            if (fallDepth > 2) {
                movementState.anticipatingFall = true;
                if (DEBUG_MODE) Chat.message(`§d[Jump] Anticipating ${fallDepth}-block fall`);
            }

            if (DEBUG_MODE) Chat.message(`§e[Jump] Gap, height diff: ${actualHeightDiff.toFixed(2)}, fall=${fallDepth}`);
        ...
        if (DEBUG_MODE) Chat.message(`§7[No Jump] Slab drop ${actualHeightDiff.toFixed(2)} blocks`);
    }
...
export function shouldJump() {
    ...
    const targetNode = splinePath[Math.min(currentNodeIndex + 2, splinePath.length - 1)];
    ...
    for (const offset of checkPoints) {
        ...
        if (isSolid(blockFoot) && !isSolid(blockHead)) {
            ...
            if (actualHeightDiff > STEP_HEIGHT) {
                if (DEBUG_MODE) {
                    Chat.message(`§e[Jump] Step up ${actualHeightDiff.toFixed(2)} at (${checkX}, ${pY}, ${checkZ})`);
                }
                movementState.lastJumpPos = { x: playerPos.x, z: playerPos.z };
                movementState.jumpType = 'step';
                return true;
...
export function detectAndRecoverFromStuck() {
    ...
    if (movementState.recoveryAttempts >= 3) {
        if (movementState.ticksWithoutProgress >= 100 && !movementState.hasRequestedRecalc) {
            movementState.hasRequestedRecalc = true;
            Chat.message(`§4[Stuck] Completely stuck for 100 ticks, requesting new path...`);

            if (global.pathEngineRecalculate) {
                global.pathEngineRecalculate();
            } else {
                Chat.message(`§c[Stuck] ERROR: Path recalculation not available!`);
            }
        }
        return;
    }
    ...
    const shouldAttemptRecovery = recoveryThresholds[movementState.recoveryAttempts] === movementState.ticksWithoutProgress;

    if (!shouldAttemptRecovery) return;

    movementState.recoveryAttempts++;
    if (DEBUG_MODE) Chat.message(`§c[Stuck Detection] Recovery attempt ${movementState.recoveryAttempts}/3 at node ${currentNodeIndex}`);
    ...
    for (let i = currentNodeIndex - 1; i >= Math.max(0, currentNodeIndex - 20); i--) {
        ...
        if (dist > 1.5 && dist < 10 && canSeePoint(node, eyePos)) {
            ...
            const angleDiff = Math.abs(((angleToNode - playerYaw + 540) % 360) - 180);

            if (angleDiff > 60) {
                if (DEBUG_MODE) Chat.message(`§e[Stuck Recovery] Retreating to node ${i} (distance: ${dist.toFixed(2)})`);
                movementState.currentNodeIndex = i;
    ...
    for (let i = Math.max(0, currentNodeIndex - 10); i < Math.min(splinePath.length, currentNodeIndex + 10); i++) {
    ...
    if (closestIndex !== currentNodeIndex && Math.abs(closestIndex - currentNodeIndex) > 2) {
        if (DEBUG_MODE) Chat.message(`§c[Stuck Recovery] Jumping to closest node ${closestIndex} (${closestDist.toFixed(2)} blocks away)`);
    ...
    for (let i = currentNodeIndex + 3; i < Math.min(splinePath.length, currentNodeIndex + 15); i++) {
        const node = splinePath[i];
        const dist = getDistance3D(playerPos, node);
        if (dist < 8 && canSeePoint(node, eyePos)) {
            if (DEBUG_MODE) Chat.message(`§6[Stuck Recovery] Skipping to visible node ${i}`);
            movementState.currentNodeIndex = i;
            movementState.lastKnownNodeIndex = i;
            movementState.recoveryLockTicks = 20;
            return;
    ...
    if (DEBUG_MODE) Chat.message(`§c[Stuck Recovery] No valid recovery nodes found`);
}
