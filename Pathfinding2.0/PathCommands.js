...
function handleCommand(args, isRustPath) {
    ...
    if (args.length < requiredCoords) {
        const usage = isRustPath ? '/rustpath <x1> <y1> <z1> <x2> <y2> <z2> [renderonly]' : '/path <x> <y> <z>';
        return global.showNotification('Invalid Command', `Usage: ${usage}`, 'ERROR', 5000);
    }
...
register('command', (...args) => handleCommand(args, true)).setName('rustpath', true);
register('command', (...args) => handleCommand(args, false)).setName('path', true);
register('command', stopPathing).setName('stop', true);

register('command', (map) => {
    if (!map) {
        return global.showNotification('Invalid Command', 'Usage: /loadmap [map]', 'ERROR', 5000);
    }
    loadMap(map);
