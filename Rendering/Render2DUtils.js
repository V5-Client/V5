...
export default class RenderLib2d {
    ...
    static drawEllipse({
        ...
        Renderer.pushMatrix()
            ...
            .enableDepth()
            .begin(
                Renderer.DrawMode.TRIANGLE_FAN,
    ...
    static drawString({
        ...
        color = Renderer.WHITE,
        shadow = false,
        backgroundColor = Renderer.getColor(0, 0, 0, 0),
        light = 15,
        align = Align.CENTER,
    }) {
        ...
        let yOffset = 0;
        const alignParts = getAlignParts(align);
        if (alignParts[0] == 'CENTER')
            y -= (TextRenderer.fontHeight / 2) * scale;
        if (alignParts[0] == 'BOTTOM') y -= TextRenderer.fontHeight * scale;
        if (alignParts[1] == 'CENTER')
            x -= (Renderer.getStringWidth(text) / 2) * scale;
        if (alignParts[1] == 'RIGHT')
            x -= Renderer.getStringWidth(text) * scale;
        ...
        ChatLib.addColor(text)
            ...
            .forEach((line, i) => {
                TextRenderer.draw(
                    ...
                    color,
                    shadow,
                    RendererUtils.getPositionMatrix(),
                    VertexConsumers,
