...
function findClass(name) {
  return java.lang.Class.forName(name, true, Player.getPlayer().getClass().getClassLoader());
}
...
register("command", () => {
  ...
  const visiblePoint = RayTrace.getPointOnBlock(blockPos, null, true);
  
  // Format output
  Chat.message(`&e===== Block Visibility Test =====`);
  Chat.message(`&7Block: &f${lookingAt.type.getName()} &7at &f[${blockPos.x}, ${blockPos.y}, ${blockPos.z}]`);
  Chat.message(`&7Distance: &f${Math.sqrt(
  ...
  if (visiblePoint) {
    Chat.message(`&7Visible Point: &f[${visiblePoint.map(v => v.toFixed(2)).join(", ")}]`);
  } else {
    Chat.message(`&7Visible Point: &cNone found`);
  }
  
  Chat.message(`&e================================`);
}).setName("checkblockvisibility").setAliases("cbv");
...
register("command", () => {
  ...
  Chat.message(`&7Auto-check mode: ${autoCheckEnabled ? "&aEnabled" : "&cDisabled"}`);
  
  if (!autoCheckEnabled) {
    lastCheckedBlock = null;
  }
}).setName("autocheckvisibility").setAliases("acv");
...
register("tick", () => {
  ...
  Chat.message(`&7[${blockPos.x}, ${blockPos.y}, ${blockPos.z}] &f${blockName}: ${
...
Chat.message("&7 - &f/checkblockvisibility &7(or &f/cbv&7) - Check current block");
Chat.message("&7 - &f/autocheckvisibility &7(or &f/acv&7) - Toggle auto-check mode");