...
class FailsafeUtils {
    ...
    sendFailsafeEmbed(type, severity, description) {
        ...
        if (mode.includes('Screenshot')) {
            Client.scheduleTask(5, () => sendFailsafeScreenshot(title, description, color, 'V5 Failsafes', ping));
            return;
        }

        sendWebhookFailsafeEmbed([{ title, description, color, footer: { text: 'V5 Failsafes' }, timestamp: new Date().toISOString() }], ping);
    }
