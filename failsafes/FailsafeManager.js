...
import { getEnabledMacros, getModule } from '../utils/MacroState';
...
class FailsafeManager {
    ...
    report(payload) {
        ...
        lines.forEach((line, idx) => chatFailsafe(line, idx === lines.length - 1));
        ...
        if (!ResponseBot.isRunning) {
            const pausedMacros = settings.pauseMacroOnFailsafe ? getEnabledMacros().map(getModule) : [];
