...
register("command", (...args) => { 
  if (!args || args.length === 0) {
    ...
    VALID_NOTIFICATION_TYPES.forEach((type, index) => {
      setTimeout(() => {
        ...
        global.showNotification(message.title, message.description, type);
      }, index * 1200);
    });
    return;
  }
  ...
  if (VALID_NOTIFICATION_TYPES.includes(potentialType)) {
    ...
    description = args.slice(2).join(" ") || "This is a sample description.";
  } else {
    // Did not specify a type or incorrect type, assume SUCCESS, /tn Title Description
    type = 'SUCCESS';
    title = args[0];
    description = args.slice(1).join(" ") || "This is a sample description.";
  }
  
  global.showNotification(title, description, type);
...
register("command", () => {
  ...
  ChatLib.chat("&e/testnotify [type] [title] [description] &7- Custom notification");
  ChatLib.chat("&bAvailable types: &f" + VALID_NOTIFICATION_TYPES.join(", "));
  ChatLib.chat("&e/tn &7- Alias for testnotify");
  ChatLib.chat("&a&m" + ChatLib.getChatBreak("-"));
}).setName("notifyhelp");