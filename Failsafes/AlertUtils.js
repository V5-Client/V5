...
class AlertUtilsClass {
     ...
     */
    triggerReaction() {
        Chat.failsafeMsg('Suspicious activity detected, reaction occuring!');
        Chat.failsafeMsg(`Press &c&l${this.cancelKey}&r &fto disable the reaction`);
        ...
        this.tracker = register('step', () => {
            if (this.cancelKeyBind.isPressed()) {
                Chat.failsafeMsg('Reaction disabled due to keybind being pressed');
    ...
    _grabWindowOnFailsafe() {
        try {
            ...
            Chat.failsafeMsg('GLFW error occured! report this.' + e);
        }
    }
}
